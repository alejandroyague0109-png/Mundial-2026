...
def link_telegram_id(user_id, chat_id):
    try:
        ...
        return True, "¡Vinculado correctamente!"
    except Exception as e:
        return False, str(e)
...
def _find_premium_matches_internal(uploaded_repes_ids, uploader_id):
    ...
    try:
        ...
        for row in response.data:
            ...
            if is_prem and chat_id:
                if chat_id not in matches: matches[chat_id] = []
                matches[chat_id].append(row['sticker_num'])
                
        return matches
    except Exception as e:
        print(f"Error buscando matches premium: {e}")
        return {}
